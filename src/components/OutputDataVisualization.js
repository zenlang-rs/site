...
const OutputDataVisualization = ({ data, darkMode }) => {
  const [expandedErrorIndex, setExpandedErrorIndex] = useState(null);

  const toggleError = (index) => {
    setExpandedErrorIndex((prevIndex) => (prevIndex === index ? null : index));
  };
  ...
  return (
    <section
      className={
        darkMode
          ? "mx-auto bg-black p-4 border-2 border-white min-h-[10rem] rounded-md"
